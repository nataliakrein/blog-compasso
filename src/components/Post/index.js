      ...
      <Card height={{ min: '30vh', max: '100%' }} width="100%" fill="horizontal" background="light-1" basis="auto">
        <CardHeader direction="column" align="start" pad="medium" gap="xsmall">
          <Box gap="medium" direction="row" align="center" fill="horizontal" justify="between">
            <Heading size="small" color="var(--title-post)" level="4">
              <Link to={`/${category}/${id}`}>{title}</Link>
            </Heading>
            <VotesIcon votes={votes} />
          </Box>
          <Text color="var(--date-post)" size="small">
            {date} by{' '}
        ...
        <CardFooter gap="xsmall" fill="horizontal" justify="evenly" margin={{ top: 'small' }} background="light-2">
          <EditButton editItem={() => history.push(`/${category}/${id}/edit/`)} />
          <DeleteButton deleteItem={() => delete_post(id)} />
          <LikeButton likeItem={() => vote(id, 'upVote')} />
          <DislikeButton dislikeItem={() => vote(id, 'downVote')} />
          <CommentButton newComment={() => history.push(`/${category}/${id}/comments/new`)} comments={comments} />
        </CardFooter>
      </Card>
    </div>
  )
