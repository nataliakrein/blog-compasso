    ...
    <div className="comment">
      <Card height={{ min: '20vh', max: '100%' }} width="95%" background="light-1" basis="auto">
        <CardHeader
          direction="column"
          align="start"
          pad={{ top: 'medium', bottom: 'xsmall', horizontal: 'medium' }}
          gap="xsmall"
        >
          <Box gap="medium" direction="row" align="center" fill="horizontal" justify="between">
            <Text color="var(--date-post)" size="small" weight="bold">
              by <Text size="small">{comment.author}</Text> on {getDate(comment.timestamp)}
            </Text>
            <VotesIcon votes={comment.voteScore} />
          </Box>
        </CardHeader>
        <CardBody height="small" pad={{ horizontal: 'medium' }}>
          <Text color="var(--text-post)" size="small">
            {comment.body}
          </Text>
        </CardBody>
        <CardFooter
          ...
          background="light-2"
        >
          <EditButton editItem={() => history.push(`/${category}/${id}/comments/${comment.id}/edit`)} />
          <DeleteButton deleteItem={() => delete_comment(comment.id)} />
          <LikeButton likeItem={() => vote_comment(comment.id, 'upVote')} />
          <DislikeButton dislikeItem={() => vote_comment(comment.id, 'downVote')} />
        </CardFooter>
      </Card>
    </div>
  )
